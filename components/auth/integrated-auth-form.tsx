...
interface IntegratedAuthFormProps {
  onSuccess?: () => void
}

export function IntegratedAuthForm({ onSuccess }: IntegratedAuthFormProps) {
  ...
  return (
    <Card className="w-full max-w-md mx-auto">
      <CardHeader className="text-center">
        ...
        <CardDescription>今すぐ体験するか、アカウントを作成してご利用ください</CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="guest" className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="guest">ゲストログイン</TabsTrigger>
            <TabsTrigger value="account">アカウント</TabsTrigger>
          </TabsList>

          <TabsContent value="guest" className="space-y-4">
            <div className="text-center text-sm text-muted-foreground mb-4">アカウント作成なしで今すぐ体験できます</div>
            <div className="space-y-3">
              ...
              >
                ...
                {isLoading ? "ログイン中..." : "日本企業として体験"}
              </Button>
              <Button
                onClick={() => handleGuestLogin("vendor")}
                className="w-full"
                variant="outline"
                disabled={isLoading}
              >
                <User className="h-4 w-4 mr-2" />
                {isLoading ? "ログイン中..." : "ベトナム企業として体験"}
              </Button>
            </div>
